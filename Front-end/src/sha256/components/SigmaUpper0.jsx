  ...
  return (
    <div className="mb-3">
      <div className="flex">
        ...
        {base === "hex" && (
          <div>
            <div className="border-b">{displayHex(input)}</div>
            <div>{displayHex(rotate2)}</div>
            <div>{displayHex(rotate13)}</div>
            <div className="border-b">{displayHex(rotate22)}</div>
            <div>{displayHex(xor)}</div>
          </div>
        )}
        {base === "bin" && (
          <div className="mr-3">
            ...
            <div>{decimalToBinary(rotate2).padStart(32, "0")}</div>
            <div>{decimalToBinary(rotate13).padStart(32, "0")} XOR</div>
            <div className="border-b mb-1">
              {decimalToBinary(rotate22).padStart(32, "0")} XOR
            </div>
            <div>{decimalToBinary(xor).padStart(32, "0")}</div>
          </div>
        )}
      </div>
    </div>
