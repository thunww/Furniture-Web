...
function MessageSchedule({ data, base, labels = [], clock, currentChunk }) {
  ...
  function focusColor(key, clock) {
    if (clock + firstLoopStartsAtClock === key && clock < 50 && clock > 1)
      return "text-white";
    if (clock - 15 + firstLoopStartsAtClock === key && clock < 50 && clock > 1)
      return "text-red-500";
    if (clock - 2 + firstLoopStartsAtClock === key && clock < 50 && clock > 1)
      return "text-green-500";
    if (clock - 16 + firstLoopStartsAtClock === key && clock < 50 && clock > 1)
      return "text-blue-500";
    if (clock - 7 + firstLoopStartsAtClock === key && clock < 50 && clock > 1)
    ...
    <div className={`duration-500 ${clock < 1 && "opacity-25"} mt-2 sm:mt-0`}>
      ...
      {data.map((word, key) => (
        <div className={`flex ${focusColor(key, clock)}`} key={key}>
          ...
          <div
            key={key.toString()}
            className={`px-1 ${focusColor(key, clock)} ${diluteOnClockOne(
              key,
              clock
            )}`}
          >
