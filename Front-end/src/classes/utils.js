...
export function hexToBinary(hex) {
  ...
  return decimal
    .map((value) => decimalToBinary(value).padStart(8, "0"))
    .join("");
}
...
export function paddingExplained(input, base = "text") {
  ...
  return {
    ...
    L64,
    result,
    inputAppended: appendOneBit(input),
  };
}
...
export function rotateRight(value, r) {
  // https://github.com/dfrankland/bitwise-rotation/blob/master/src/index.js
  const bitWidth = 32;
  const rotation = r & (bitWidth - 1);
  const bitMask = 2 ** bitWidth - 1;
  return (value >>> rotation) | ((value << (bitWidth - rotation)) & bitMask); // Change from signed to unsigned
}
